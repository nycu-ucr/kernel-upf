...
Status GtpTunnelAddQer(const char *ifname, struct gtp5g_qer *qer) {
    ...
    UTLT_Assert(!gtp5g_add_qer(info.genl_id, 
								info.nl, 
								dev, 
								qer), 
				status = STATUS_ERROR,
...
Status GtpTunnelModQer(const char *ifname, struct gtp5g_qer *qer) {
    ...
    UTLT_Assert(!gtp5g_mod_qer(info.genl_id, 
								info.nl, 
								dev, 
								qer), 
				status = STATUS_ERROR,
...
Status GtpTunnelDelQer(const char *ifname, uint32_t id) {
    ...
    UTLT_Assert(!gtp5g_del_qer(info.genl_id, 
								info.nl, 
								dev, 
								qer), 
				status = STATUS_ERROR,
...
struct gtp5g_qer *GtpTunnelFindQerById(const char *ifname, uint32_t id) {
    ...
    UTLT_Assert((rt_qer = gtp5g_qer_find_by_id(info.genl_id, 
											info.nl, dev, qer)), ,
